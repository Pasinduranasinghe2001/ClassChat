...
public class ClassChatClient {
    ...
    private void setupGUI() {
        frame = new JFrame("ClassChat (Task2)");
    ...
    private void connectToServer() {
        try {
            username = JOptionPane.showInputDialog(
                    frame,
                    "Enter your username:",
                    "Welcome to ClassChat (Task2)",
                    JOptionPane.PLAIN_MESSAGE
            );

            if (username == null || username.trim().isEmpty()) username = "Anonymous";
            frame.setTitle("ClassChat - " + username + " (Task2)");

            socket = new Socket(SERVER_ADDRESS, SERVER_PORT);
            in = new BufferedReader(new InputStreamReader(socket.getInputStream()));
            out = new PrintWriter(socket.getOutputStream(), true);

            //  Task2: inform server about username
            out.println("/join " + username);

            //  Duplex receive thread (low CPU)
            new Thread(() -> {
                try {
                    String message;
    ...
    private void disconnect() {
        try {
            if (out != null && username != null) out.println("/leave " + username);
